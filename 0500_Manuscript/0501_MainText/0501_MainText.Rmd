...
Most of the world's population live in low- and middle-income countries (LMICs), where access to gold-standard diagnostics for SARS-CoV-2, like PCR, is limited. Syndromic diagnosis is often the only means to identify cases, even though many are asymptomatic. Rapid antigen testing is an inexpensive and easy-to-deploy surveillance alternative but concerns remain about sensitivity and specificity. Here we [[[[....DO SOMETHING REALLY USEFUL AND IMPORTANT - TELL ME WHAT!!!!...]]] to effectively guide practice.

*Methods*

Community-based volunteers were trained in Dhaka, Bangladesh to identified potential COVID-19 patients through syndromic surveillance. 
A sample (n = `r pop_size`) of these patients was tested using PCR and RAT. Statistical models were fit to predict their PCR status using only RAT results, only syndromic data, and their combination. The predictive power and classification performance of these models was examined under relevant epidemiological scenarios experienced in Dhaka during the pandemic: "Agnostic", "Rising Cases" and "Low-Level Cases". [[[[.. WE STILL NEED A BIT MORE HERE - THE PURPOSE OF THE COMPARISON WAS TO EVALUATE WHAT???..]]]

*Findings*

Models combining RAT and syndromic data yielded equal or improved performance over syndromic- and RAT-only models across all scenarios and by scenario-free prediction and classification metrics.  
In the "Rising Cases" scenario, which represents the current situation in many LMICs, the combined data model's false negative rate is `r 100*(RATonlyFNR - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>% select(MedFalseNegRate) %>% unlist(use.names = FALSE) %>% min()) %>% round(2)` (IQR: `r 100*(RATonlyFNR - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>%  select(FalseNegRate_CI_0.75) %>%  unlist(use.names = FALSE) %>%  min()) %>% round(2)`-`r 100*(RATonlyFNR - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>% select(FalseNegRate_CI_0.25) %>% unlist(use.names = FALSE) %>% min()) %>%  round(2)`) percentage points lower than the RAT only model's. [[[[..  WE NEED TO REPORT SOME MORE RESULTS COMPARING ACROSS SCENARIOS..]]]

*Interpretation*

By drawing from complementary strengths across two imperfect diagnostics, we demonstrate substantive improvements in SARS-CoV-2 detection, and reduced rates of false positive diagnostics [[[[..  DETAIL THE PROS AND CONS OF THIS FOR PRACTITIONERS & PATIENTS HERE; GIVE A VISION OF HOW THIS COULD BE DEPLOYED..]]]. Small, scalable improvements in the accuracy of current imperfect diagnostic practices have potential to better control the pandemic in LMIC settings.  
...
The faster and more accurately we can identify cases, the more effectively we can provide clinical care, reduce transmission of infection and implement population-level interventions.
Reverse transcription polymerase chain reaction (RT-PCR, hereafter, PCR) testing has rapidly become the default, gold-standard test for SARS-CoV-2 [[[[..  maybe check how COVID-19 vs SARS-CoV-2 used in other Global Health papers.]]] in applied settings (although see [@drame2020should]) due to its high sensitivity and specificity [@corman2020detection, @tahamtan2020real].
Most of the world's population, however, live in low- and middle-income countries (LMICs), where laboratory facilities needed to carry out PCR tests are often scarce and hard to access [@chowdhury2020long, @vandenberg2021considerations]. 
In such settings, patient diagnosis and support is limited. In Dhaka, Bangladesh, some telemedicine or community support teams (CSTs) composed of local volunteers with basic training in COVID-19 diagnosis and care and prevention of transmission.
COVID-19 diagnosis worldwide, therefore, must become accessible, harnessing inexpensive methods that can be carried out locally [@cash2020has, @olalekan2020covid].

An increasingly popular alternative to PCR is rapid antigen testing (RAT) [@linares2020panbio].
Like PCR, these tests have high specificity for COVID-19 while being less expensive, easier to implement, and faster but with lower sensitivity [@boum2021performance].
...
Together, these traits make RATs an appealing alternative to PCR, however, concerns have been raised that the lower sensitivity of RAT [@mak2020evaluation, muhi2021multi] leads to unacceptably many false negative diagnoses.
...
In this approach, rather than using a set of pre-determined thresholds, a range of symptomatic and risk factor data (such as age and gender ) are collected and then a sub-sample of patients is tested using PCR for validation [@sim2021utilizing].
...
Although the syndromic only model (Model Class 2) matches the combined model's false negative rate, its false positive rate is `r 100*(scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndOnly") %>% select(MedError) %>% unlist(use.names = FALSE) %>% min() - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>% select(MedError) %>% unlist(use.names = FALSE) %>% min()) %>% round(2)` (IQR: `r 100*(scenario_outcomes %>% filter(scenario == "Scenario 2") %>%  filter(ModelClass == "SyndOnly") %>% select(Error075) %>% unlist(use.names = FALSE) %>% min() - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>% select(Error075) %>% unlist(use.names = FALSE) %>%  min()) %>% round(2)`- `r 100*(scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndOnly") %>% select(Error025) %>%  unlist(use.names = FALSE) %>%  min() - scenario_outcomes %>% filter(scenario == "Scenario 2") %>% filter(ModelClass == "SyndRAT") %>% select(Error025) %>%  unlist(use.names = FALSE) %>% min()) %>% round(2)`) percentage points higher.
...
Furthermore, this boost in diagnostic performance is achieved with data that are either already being collected or are currently being rolled out in Bangladesh and other LMICs [@aziz2020integrated, @schultz2021pragmatic]. 
...
This could be explored by using viral load measured as Threshold Cycle (Ct) values from the PCR [@albert2021field] and further testing for other illnesses.
