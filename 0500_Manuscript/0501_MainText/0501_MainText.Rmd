...
knitr::include_graphics(paste0(getwd(),"/0500_Manuscript/MainTextFigs/ModelSelectionFlowchart.pdf"))
```
...
In Scenario 1 ("Agnostic", see  \@ref(tab:scenarios-tab).), the median error was 0.62 for models in Class 1 and Class 3 and between 0.90 and 0.96 for models in Class 2 (Figure \@ref(fig:scenario-plot)).
...
In Scenario 3 ("Steady, Low-Level Cases"), the error in Class 1 was 0.60 and the median errors ranged from 0.70 to 0.75 for Class 2, and 0.44 and 0.47 for Class 3 (Figure \@ref(fig:scenario-plot)).
...
In the most relevant scenario, 2 ("Rising Cases"), where we try to keep the false negative rate low, the combined data model achieves 40 percentage points lower than the false negative rate of the RAT only model, and a 12 percentage points lower false positive rate than the syndromic only model.
