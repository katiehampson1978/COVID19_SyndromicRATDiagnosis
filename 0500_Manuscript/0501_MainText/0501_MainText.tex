...
In contract, RAT can be conducted on nasal or saliva samples, completed in the home and results are available in 30 minutes.
...
However, rather than using a set of pre-determined criteria, a range of symptomatic and risk factor data are collected and then a sub-sample of patients are tested using RT-PCR for COVID-19 {[}11{]}.
...
Poor sensitivity and specificity are problematic in diagnositics but higher error rates may be tolerable depending on their scale and impact given the local situation.
...
When the prevalence of the disease is low, false positives may create local skepticism about the value of testing, or when there are strong population-level mitigations already in place (such as a nationwide lockdown), then false positives might be more costly than false negatives {[}16{]}.
...
The large number of symptoms means that there is a high number of potential model configurations (\textgreater131000 for 14 symptoms and two covariates) which might, by chance, perform well on the test sets (even under the challenging conditions of temporal cross validation) but lack transferability.
...
Cross entropy measures the accuracy of probabilistic predictions for models that predict binary outcomes using probabilities {[}20{]}.
...
In Scenario 1, the median error was 0.62 for models in Class 1 and Class 3 and between 0.90 and 0.96 for models in Class 2 (Figure \ref{fig:scenario-plot}).
In Scenario 2, Model Class 1 was unable to meet the required false negative rate.
The median errors were between 0.69 and 0.74 for models in Class 2, and 0.57 and 0.69 for models in Class 3 (Figure \ref{fig:scenario-plot}).
In Scenario 3, the error in Class 1 was 0.60 and the median errors ranged from 0.70 to 0.75 for Class 2, and 0.44 and 0.47 for Class 3 (Figure \ref{fig:scenario-plot}).

\begin{figure}
\centering
\includegraphics{0501_MainText_files/figure-latex/ROC-plot-1.pdf}
\caption{\label{fig:ROC-plot} Receiver operating characteristics for RAT only approach and posterior mean (+- posterior standard deviation) receiver operating characteristics for Class 2 and 3 models. These curves demonstrate the performance of the model for any given scenario as defined by false and true positive rates (as opposed to Figure 5 which demonstrates model performance in specific scenarios).}
\end{figure}
...
We propose that this is due to the fact that RAT is most effective during the first week of symptom onset, with much worse performance pre- and post- this period {[}21{]}.
