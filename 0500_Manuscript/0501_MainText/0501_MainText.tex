...
A sample (n = \text{1172}) of these patients was tested using RAT and syndromic data were collected.
...
In the ``Rising Cases'' scenario, which most closely represents the current situation in many LMICs, the combined data model false negative rate is \text{26} (IQR: \text{24}-\text{29}) percentage points lower that of the RAT only model.
...
Although the syndromic only model (Model Class 2) matches the combined models false negative rate, its false positive rate is \text{31} (IQR: \text{29}- \text{34}) percentage points higher.
...
The Bill and Melinda Gates Foundation funded work by FAO (INV-022851), University of Glasgow reports funding from Wellcome (207569/Z/17/Z) and the Engineering and Physical Sciences Research Councils. \todo{Find Fergus PhD funding code}
