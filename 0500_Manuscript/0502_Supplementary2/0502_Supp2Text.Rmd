...
\todo{Find more ways to cite Figure \@ref(fig:data-flowchart2) in this text.}
...
Structurally, the model combines Equations \@ref(eq:ModelClass1) and \@ref(eq:ModelClass2), with RAT-positive patients being modelled using Equation \@ref(eq:ModelClass1), and Equation \@ref(eq:ModelClass2).
...
\todo{This isn't quite right - need to demonstrate subsetting of i's, otherwise it implies that z is calculated for all data points}
...
However, if more time were available for model development we would advocate more sophisticated methods of variable selection, such as penalised complexity priors [@simpson2017penalising] or auxiliary variable selection [@held2006bayesian].
...
$$
\todo{I'm not sure if any of these equations are correct or sufficient or clear}
